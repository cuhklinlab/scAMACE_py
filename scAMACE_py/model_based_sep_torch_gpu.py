...
def pst_rna_gpu(phi_rna, g_1, g_0,w_rna,pi_rna,alpha_1=2, beta_1=2):
...
def EM_rna_gpu(g_1, g_0,phi_rna,w_rna,pi_rna,alpha_1 = 2,beta_1 = 2,max_iter = 200,flag = True):
    pst = []
    for i in range(max_iter):
        E_z_rna,E_z_u_rna,E_z_1_u_rna,E_z_u_v_rna,E_z_1_u_v_rna,E_z_1_u_v_rna = cal_E_gpu(g_1,g_0,phi_rna,w_rna,pi_rna)
        K = E_z_rna.shape[0]
        ...
        print(i,'done')
        if flag == True:
            tmp_pst = pst_rna_gpu(phi_rna, g_1, g_0,w_rna,pi_rna,alpha_1,beta_1)
            pst.append(tmp_pst)
            print(i,'pst done')

    if flag == False:
        pst.append(pst_rna_gpu(phi_rna, g_1, g_0,w_rna,pi_rna,alpha_1,beta_1))
...
def pst_atac_gpu(f_1,f_0,phi_atac,w_atac,qi,alpha_1=2, beta_1=2):
    ...
    #prior
    res += torch.sum(torch.log(phi_atac)) + torch.sum(torch.log(qi) + torch.log(1 - qi))
...
def cal_E_atac_gpu(f_1,f_0,phi_atac,w_atac,qi):
    n_atac = f_0.shape[0]
#     if type(qi) is float:
...
def EM_atac_gpu(f_1,f_0,phi_atac,w_atac,qi,alpha_1=2,beta_1 = 2,
    alpha_qi = 2, beta_qi = 2,max_iter = 200,
    flag = True):
    pst = []
    for i in range(max_iter):

        E_z_atac,E_z_u_atac,E_z_u_u_t_atac = cal_E_atac_gpu(f_1,f_0,phi_atac,w_atac,qi)
        ...
        #update w_atac
        w_atac_n = (torch.sum(E_z_u_atac,dim = 1) + alpha_1  - 1 ) / (torch.sum(E_z_atac,dim = 1)[:,None] + alpha_1 + beta_1 - 2)    
        ...
        print(i,'done')
        if flag == True:
            tmp_pst = pst_atac_gpu(f_1,f_0,phi_atac,w_atac,qi,alpha_1,beta_1)
            pst.append(tmp_pst)
            #print(tmp_pst)
            print(i,'pst done')

    if flag == False:
        pst.append(pst_atac_gpu(f_1,f_0,phi_atac,w_atac,qi,alpha_1,beta_1))
...
def pst_met_gpu(h_1,h_0,phi_met,w_met,pi_met,alpha_1=2,beta_1=2):
    res = 0
    n_met = h_1.shape[0]
...
def EM_met_gpu(h_1,h_0,phi_met,w_met,pi_met,alpha_1 = 2,beta_1 = 2,max_iter = 200,flag = True):
    pst = []
    ...
    for i in range(max_iter):
        ...
        #updata w_met
        w_met_n = (torch.sum(E_z_u_met,dim = 1) + alpha_1  - 1 ) / (torch.sum(E_z_met,dim = 1)[:,None] + alpha_1 + beta_1 - 2)
        ...
        if flag == True:
            tmp_pst = pst_met_gpu(h_1,h_0,phi_met,w_met,pi_met,alpha_1,beta_1)
            pst.append(tmp_pst)
            print(i,'pst done')


    if flag == False:
        pst.append(pst_met_gpu(h_1,h_0,phi_met,w_met,pi_met,alpha_1,beta_1))
