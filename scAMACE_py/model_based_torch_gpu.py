...
def cal_E_gpu(g_1, g_0, phi_rna, w_rna, pi_rna):
    # calculate E_z_lk   k*l'
    add_1 = pi_rna[0][:, None] * g_1 + (1 - pi_rna[0][:, None]) * g_0
    add_2 = pi_rna[1][:, None] * g_1 + (1 - pi_rna[1][:, None]) * g_0

    prod_1 = cal_prod_gpu(w_rna, add_1)
    prod_2 = cal_prod_gpu(1 - w_rna, add_2)
...
def E_step_gpu(f_1, f_0, phi_atac, w_atac, qi,
    ...
    qf = qi * f_1 + (1 - qi) * f_0
    # print('qf',qf.device)
    prod_1 = cal_prod_gpu(w_atac, qf)
    # print(prod_1.device)
    prod_2 = cal_prod_gpu(1 - w_atac, f_0)
    # print(prod_2.device)
    Log_E_z_atac = torch.log(prod_1 + prod_2).sum(2)
    # Log_E_z_atac = torch.sum(torch.log(prod_1+prod_2),dim = torch.tensor(2)) # result dimension k * n_atac
    Log_E_z_atac = Log_E_z_atac - torch.max(Log_E_z_atac, dim=0)[0] + 20
    # print(Log_E_z_atac.device)
    # print(phi_atac[:,None].device)
    ...
    E_z_rna, E_z_u_rna, E_z_1_u_rna, E_z_u_v_rna, E_z_1_u_v_rna, E_z_1_u_v_rna = cal_E_gpu(g_1, g_0, phi_rna,
                                                                                       w_rna, pi_rna)

    E_z_met, E_z_u_met, E_z_1_u_met, E_z_u_v_met, E_z_1_u_v_met, E_z_1_u_v_met = cal_E_gpu(h_1, h_0, phi_met,
                                                                                       w_met, pi_met)
...
def update_w_rna_linked_gpu(K, p, E_z_u_rna, E_z_1_u_rna, w_atac, w_met,
    ...
    to_op = scsp.beta(tmp_c.cpu(), (phi_1 - tmp_c).cpu())
    to_op = to_op.cuda()
    to_max -= torch.log(to_op)

    mu_2 = cal_mu_lin(w_rna, delta=delta, theta=theta)
    tmp_c = mu_2 * phi_2
    to_max += (tmp_c - 1) * torch.log(w_met)[:, :, None] + (phi_2 - tmp_c - 1) * torch.log(1 - w_met)[:, :, None]
    to_op = scsp.beta(tmp_c.cpu(), (phi_2 - tmp_c).cpu())
    to_op = to_op.cuda()
    # tmp_log = torch.log(scsp.beta(tmp_c, phi_2 - tmp_c ))
    # tmp_log = tmp_log.cuda()
    to_max -= torch.log(to_op)
...
def M_step_gpu(E_z_atac, E_z_u_atac, E_z_u_u_t_atac,
    ...
    # update w_rna linked part via grid search a function to do so
    w_rna = update_w_rna_linked_gpu(K, p, E_z_u_rna, E_z_1_u_rna, w_atac, w_met, alpha_1=alpha_1, beta_1=beta_1,
...
def cal_pst_gpu(phi_atac, f_1, f_0,
            phi_rna, g_1, g_0,
            phi_met, h_1, h_0,
    ...
    a = cal_prod_gpu(w_atac, qi[:, None] * f_1)
    b = cal_prod_gpu(w_atac, qi[:, None] * f_0)
    res = a + f_0 - b
    ...
    # for rna data
    a = cal_prod_gpu(w_rna, pi_rna[0][:, None] * g_1) + cal_prod_gpu(1 - w_rna, pi_rna[1][:, None] * g_1)
    a += cal_prod_gpu(w_rna, (1 - pi_rna[0])[:, None] * g_0)
    a += cal_prod_gpu(1 - w_rna, (1 - pi_rna[1])[:, None] * g_0)
    ...
    # for met data
    n_met = h_1.shape[0]
    a = cal_prod_gpu(w_met, pi_met[0][:, None] * h_1) + cal_prod_gpu(1 - w_met, pi_met[1][:, None] * h_1)
    a += cal_prod_gpu(w_met, (1 - pi_met[0])[:, None] * h_0)
    a += cal_prod_gpu(1 - w_met, (1 - pi_met[1])[:, None] * h_0)

    # overflow may happen
    # b = phi_met[:,None] * np.prod(a,dim = 2)
    # tmp_pst = np.sum(np.log(np.sum(b,dim = 0 )))
    # pst.append(tmp_pst)
    ...
    to_op = scsp.beta(tmp_c.cpu(), (phi_1 - tmp_c).cpu())
    to_op = to_op.cuda()
    tmp_pst -= torch.sum(torch.log(to_op))
    tmp_c = mu_2 * phi_2
...
def EM_gpu(f_1, f_0, g_1, g_0, h_1, h_0,
    ...
    for i in range(max_iter):
        (E_z_atac, E_z_u_atac, E_z_u_u_t_atac,
         E_z_rna, E_z_u_rna, E_z_1_u_rna, E_z_u_v_rna, E_z_1_u_v_rna, E_z_1_u_v_rna,
         E_z_met, E_z_u_met, E_z_1_u_met, E_z_u_v_met, E_z_1_u_v_met, E_z_1_u_v_met) = E_step_gpu(f_1, f_0, phi_atac,
                                                                                              w_atac, qi, g_1, g_0,
        ...
        phi_atac_n, phi_rna_n, phi_met_n, qi_n, pi_rna_n, pi_met_n, w_rna_n, w_atac_n, w_met_n = M_step_gpu(E_z_atac,
                                                                                                        E_z_u_atac,
                                                                                                        E_z_u_u_t_atac,
                                                                                                        E_z_rna,
                                                                                                        E_z_u_rna,
        ...
        if flag == True:
            ...
            tmp_pst = cal_pst_gpu(phi_atac, f_1, f_0,
                              phi_rna, g_1, g_0,
                              phi_met, h_1, h_0,
                              qi, pi_rna, pi_met, w_rna, w_atac, w_met,
